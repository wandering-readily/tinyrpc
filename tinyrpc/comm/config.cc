...
void Config::readLogConfig(TiXmlElement* log_node) {
  ...
  printNodeAndGetTextErrorIfExist(node, "app_log_level");
...
void Config::readConf() {
  ...
  printNodeAndGetTextErrorIfExist(coroutine_stack_size_node, 
  ...
  printNodeAndGetTextErrorIfExist(bucket_num_node, "bucket_num");
  TiXmlElement *inteval_node = 
      time_wheel_node->FirstChildElement("inteval");
  printNodeAndGetTextErrorIfExist(inteval_node, "inteval");
...
void Config::printNodeErrorIfExist(TiXmlElement *pNode, const char *pattern) {
  if(!pNode) {
    printf("start tinyrpc server error! read config file [%s] error, "
            "cannot read [%s] xml node\n", m_file_path.c_str(), pattern);
    exit(0);
